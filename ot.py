...
def mostrar_dashboard_ot(archivo_subido):
    ...
    try:
        ...
        # Asumimos que existe una columna 'Date' ya que pediste agrupar por fecha
        ...
        # 6. Gráficos y Divisiones
        c1, c2 = st.columns(2)
        
        with c1:
            st.subheader("Total Pagado por Fecha")
            # Agrupar por fecha sumando el total a pagar
            ...
            fig_date = px.bar(
                ...
                x="Date", 
                y="Total a Pagar", 
                title="Distribución de Pagos por Día", 
                text_auto='.2s'
            )
        ...
        with c2:
            ...
            # Agrupar por agente sumando el total a pagar
            df_agent = df_ot.groupby("Agent", as_index=False)["Total a Pagar"].sum()
            # Ordenar de mayor a menor para mejor visualización
            df_agent = df_agent.sort_values(by="Total a Pagar", ascending=False)
            ...
            fig_agent = px.bar(
                ...
                y="Total a Pagar", 
                color="Agent",
                title="Pago Acumulado por Agente"
            )
        ...
        # Crear los DataFrames resumidos que irán en el Excel
        # Resumen por Agente (Mostrando horas totales y pago total)
        df_excel_agent = df_ot.groupby("Agent", as_index=False).agg({
            "Hours": "sum",
            "Total a Pagar": "sum"
        }).rename(columns={"Hours": "Total Horas Trabajadas"})
        
        # Resumen por Fecha
        df_excel_date = df_ot.groupby("Date", as_index=False).agg({
            "Hours": "sum",
            "Total a Pagar": "sum"
        }).rename(columns={"Hours": "Total Horas Aprobadas"})

        # Vista previa en pantalla
        st.dataframe(df_excel_agent, use_container_width=True)
        
        def generar_excel_ot(df_agentes, df_fechas, df_crudo):
            output = BytesIO()
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                df_crudo.to_excel(writer, index=False, sheet_name='Detalle Calculado') # El histórico completo
                
                # Ajuste de ancho de columnas
                for sheet_name in ['Total por Agente', 'Total por Fecha', 'Detalle Calculado']:
                    worksheet = writer.sheets[sheet_name]
                    worksheet.set_column(0, 5, 20)
                    
            return output.getvalue()
            
        excel_resultados_ot = generar_excel_ot(df_excel_agent, df_excel_date, df_ot)
